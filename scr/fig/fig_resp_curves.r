...
par(mfrow=c(5, 2), bty='n', mar=c(2,2,0,0), mgp=c(1,0.25,0), oma=c(2,2,0,0), tcl=-0.2, cex.axis=0.5)
cex.xtitles = 0.5
cex.ytitle = 0.75
cex.title = 0.5
line.title = -1
...
mtext("Mean Annual Temperature (°C)", side=1, outer=TRUE, cex=cex.ytitle, line=0)
...
mtext("Mean Annual Precipitation (mm)", side=1, outer=TRUE, cex=cex.ytitle, line=0)
mtext(expression("Probability of " * phantom(bold(" colonization")) * "/" * 
		phantom(bold("extinction"))),col="black", side=2, outer=TRUE, cex=cex.ytitle, line=0)
mtext(expression(phantom("Probability of ") * bold(" colonization") * phantom("/") * 
		phantom(bold("extinction"))),col=col.color, side=2, outer=TRUE, cex=cex.ytitle, line=0)
